...
public class MainActivity2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView valeurField = (TextView) findViewById(R.id.valeurField);
        String valeurs= "Valeur : " ;
        /* A)
        SharedPreferences settings = getSharedPreferences("cycle_vie_prefs", Context.MODE_PRIVATE);
        valeurs += settings.getString("cle", "");
        */

        /* B)
        Intent intent = getIntent();
        if (intent != null)  valeurs+=intent.getStringExtra("clé") ;
        */

        // C)
        if (savedInstanceState != null){
            valeurs += savedInstanceState.getString("cle");
        }

        valeurField.setText(valeurs);
    }
    /** =============================================================
